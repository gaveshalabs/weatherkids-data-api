...
export class KitePlayersService {
  ...
  private getRandomAvatarUrl(): string {
    const randomFilename =
      this.avatarFilenames[
        Math.floor(Math.random() * this.avatarFilenames.length)
      ];
  ...
  async findKitePlayerByUserId(_id: string): Promise<KitePlayer> {
    const kitePlayer = await this.kitePlayerModel
      .findOne({ user_id: _id })
      .exec();
    if (!kitePlayer) {
    ...
    const pipeline = [
      ...
      },
      {
        $group: {
          ...
          unique_attempt_timestamps: {
            $addToSet: '$kite_data.metadata.attempt_timestamp',
          },
        },
      },
      {
